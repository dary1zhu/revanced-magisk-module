...
get_prebuilts() {
	...
	for src_ver in "$cli_src CLI $cli_ver cli" "$patches_src Patches $patches_ver patches"; do
		...
		if [ -z "$file" ]; then
			...
			matches=$(jq -e --arg prefix "$fprefix" '.assets | map(select(.name | ascii_downcase | (contains($prefix) or endswith(".jar"))) | select(.name | (endswith(".jar") or endswith(".mpp"))))' <<<"$resp") || return 1
...
req() { _req "$1" "$2" -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36"; }
...
patch_apk() {
	...
	# 核心改动：
	# 1. 增加 -Xmx2g 限制内存
	# 2. 增加 --force-apktool 提高解析兼容性
	# 3. 增加 --continue-on-error 防止单个补丁失败导致整体熔断
	local cmd="java -Xmx2g -jar '$cli_jar' patch '$stock_input' --purge -o '$patched_apk' -p '$patches_jar' \
--keystore=ks.keystore --keystore-entry-password=123456789 --keystore-password=123456789 \
--signer=jhc --keystore-entry-alias=jhc --force-apktool --continue-on-error $patcher_args"
...
check_sig() {
	...
	if grep -q "$pkg_name" sig.txt; then
	    pr "Calculating SHA-256 for $pkg_name... Please wait." >&2
...
list_args() {
    # 1. 尝试用 jq 将其作为 JSON 数组解析并输出每一行
    # 2. 如果解析失败（比如传入的是纯字符串），则退回到基础清理逻辑
    if ! echo "$1" | jq -e -r 'if type=="array" then .[] else . end' 2>/dev/null; then
        # 备用方案：仅移除外层的引号和空格
